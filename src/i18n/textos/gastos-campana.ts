...
export const gastosCampanaPt: typeof gastosCampanaEs = {
  ...
  historial: {
    ...
    parteDeLiquidacionDetalle: 'Se mexe a partir do lote: não se edita nem se anula solta. Anule a liquidação inteira.',
    repartido: (n: number) => `Dividido em ${n} lotes`,
    anularJuntas: (n: number) => `Anular as ${n} juntas`,
    anularJuntasPregunta: (n: number) =>
      `Esta despesa está dividida em ${n} lotes. As ${n} partes são anuladas juntas. Seguimos?`,
    anuladasJuntas: (n: number) => `Pronto: as ${n} partes foram anuladas.`,
  },
